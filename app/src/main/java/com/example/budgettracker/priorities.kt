...
//recycler view of the priorities in the database
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        ...
        // keep bottom space so floating main button won't overlap items
        val bottomPadding = resources.getDimensionPixelSize(R.dimen.fab_bottom_padding)
        recycler.setPadding(recycler.paddingLeft, recycler.paddingTop, recycler.paddingRight, bottomPadding)
        recycler.clipToPadding = false

        // swipe to delete (also delete transactions related)
        val itemTouchHelper = ItemTouchHelper(object : ItemTouchHelper.SimpleCallback(0, ItemTouchHelper.LEFT) {
            ...
            override fun onSwiped(viewHolder: RecyclerView.ViewHolder, direction: Int) {
                ...
                db.collection("transactions")
                    ...
                    .addOnFailureListener {
                        // couldn't query transactions, restore item
                        adapter.notifyItemChanged(pos)
                    }
            }
        })
        itemTouchHelper.attachToRecyclerView(recycler)
//important to fetch data
        fetchData()
        return view
    }
//clearing all the priority deleted's data
